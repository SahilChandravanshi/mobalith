...
function scoreDuplicateRoles(
  ...
  const duplicates = hero.roles.filter((role) =>
    allyRoles.includes(role),
  );
...
function scoreDamageBalance(
  ...
  const magic = allyTeam.filter(
    (h) => h?.damageType === "Magic",
  ).length;

  const physical = allyTeam.filter(
...
function scoreLaneBalance(
  ...
  const freeLane = hero.lanes.find(
...
export async function getRecommendations(
  ...
  const relationships =
    await relationshipRepository.getAll();
    ...
    .map((hero) => {
      ...
      if (relation) {
        enemyTeam.forEach((enemy) => {
          ...
          const counter = relation.counters.find(
            (c) => c.heroId === enemy.id,
          );
        ...
        allyTeam.forEach((ally) => {
          ...
          const synergy = relation.synergies.find(
            (s) => s.heroId === ally.id,
          );
